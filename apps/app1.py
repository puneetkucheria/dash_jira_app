...
def get_projects(one='',two=''):
    df= pd.DataFrame( [row.project.key for index,row in pd.DataFrame(jira_s.projects(), columns=['project']).iterrows()],columns=['project'])
...
def project_dd(portfolio,subportfolio):
    ...
    return [{'label':project_list.iloc[i]['project'], 'value':project_list.iloc[i]['project']} for i in range(len(project_list))]
...
def project_table(project_id):
    #return html.Table(
        # Header
#        [html.Tr([html.Th(col) for col in[['Issue]] ])] +
...
#)
    return html.Div('getting project details for '+project_id) if project_id else html.Div('No project Selected')
