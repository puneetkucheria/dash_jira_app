...
def get_project_details(project):
    ...
    #    print(issues.columns)
        ...
        issue_type_count.rename(columns={'Issue Type':'Issue Count'}, inplace=True)
        issue_type_count['Issue Type']=issue_type_count.index

        # Calculate issue status table
        issue_status_count=pd.DataFrame(issues['Status'].value_counts())
        issue_status_count.rename(columns={'Status':'Issue Count'}, inplace=True)
        issue_status_count['Status Type']=issue_status_count.index
    #    print(issue_status_count)
    #    print(gh.project(project))
    else:
        return [],[],[]
...
@app.callback(
        ...
        [Input('project_dd','value'),
         Input('subportfolio_dd','value'),
         Input('portfolio_dd','value')])
def project_table(project_id,sub,port):
